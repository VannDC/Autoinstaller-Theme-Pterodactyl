...
check_token() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]               LICENSE VANZ RYUICHI            [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  echo -e "${YELLOW}MASUKKAN AKSES TOKEN: ${NC}"
  read -r USER_TOKEN

  if [ "$USER_TOKEN" = "vsunseeCreated" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di Vanz Ryuichi${NC}"
    echo -e "${YELLOW}TELEGRAM : @vanzryuichi${NC}"
    echo -e "${YELLOW}WHATSAPP : +6288276746473${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 10K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© VANZ RYUICHI${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  while true; do
    ...
    case "$SELECT_THEME" in
      1)
        THEME_URL=$(echo -e "https://github.com/VannDC/Autoinstaller-Theme-Pterodactyl/raw/main/stellar.zip")  
        break
        ;;
      2)
        THEME_URL=$(echo -e "https://github.com/VannDC/Autoinstaller-Theme-Pterodactyl/raw/main/billing.zip")
        break
        ;;
      3)
        THEME_URL=$(echo -e "https://github.com/VannDC/Autoinstaller-Theme-Pterodactyl/raw/main/enigma.zip")
        break
        ;; 
      x)
        return
...
# Unduh file tema
wget -O /root/stellar.zip https://github.com/VannDC/Autoinstaller-Theme-Pterodactyl/raw/main/stellar.zip
...
install_nebula_theme() {
    ...
    local REPO_URL="https://github.com/VannDC/Autoinstaller-Theme-Pterodactyl.git"
...
install_elysium_theme() {
    ...
    REPO_URL="https://github.com/VannDC/Autoinstaller-Theme-Pterodactyl.git"
...
installTheme_nightcore(){
    ...
    git clone https://github.com/VannDC/Autoinstaller-Theme-Pterodactyl.git > /dev/null 2>&1
