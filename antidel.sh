...
echo -e "${CYAN}${BOLD}"
echo "╔══════════════════════════════════════════════════════╗"
echo "║         Pablo×Vanzz Protect + Panel Builder         ║"
...
if [ "$OPSI" = "1" ]; then
    ...
    in_func == 1 && /^\s*{/ {
        ...
        print "            throw new DisplayException(\"Anda Bukan Lah Admin Utama. Anda Tidak Bisa Mendelete User Lain (Pablo×Vanzz Protect V'"$VERSION"')\");";
        print "        }";
        in_func = 0; next;
    }
    { print }
...
' "$SERVICE_SERVER" > "$SERVICE_SERVER.tmp" && mv "$SERVICE_SERVER.tmp" "$SERVICE_SERVER"
    ...
    in_func == 1 && /^\s*{/ {
        ...
        print "            throw new DisplayException(\"Anda Bukan Lah Admin Utama. Anda Tidak Bisa Menghapus Server Ini (Pablo×Vanzz Protect V'"$VERSION"')\");";
        print "        }";
        in_func = 0; next;
    }
    { print }
